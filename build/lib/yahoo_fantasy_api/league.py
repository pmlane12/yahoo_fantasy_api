...
class League:
        ...
        for e in t.execute('$..(full,player_id,position_type,stat)'):
            if 'player_id' in e:
                ...
                if stat_id in self.stats_id_map:
                    row[self.stats_id_map[stat_id]['display_name']] = val
        if row is not None:
            stats.append(row)
        return stats
    ...
    def _cache_stats_id_map(self, game_code):
        ...
        if self.stats_id_map is None:
            ...
            stats_id_map = {}
            for s in t.execute('$..stat.(stat_id,display_name,sort_order,is_only_display_stat)'):
                if 'is_only_display_stat' in s:
                    stats_id_map[int(s['stat_id'])] = {'display_name': s['display_name']
                                                        , 'sort_order': s['sort_order']
                                                        , 'is_only_display_stat': s['is_only_display_stat']}
                else:
                    stats_id_map[int(s['stat_id'])] = {'display_name': s['display_name']
